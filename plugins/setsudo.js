...
cmd({
    ...
    react: "😇",
    filename: __filename
}, async (conn, mek, m, { from, args, q, isCreator, isOwner, reply }) => {
    try {
        ...
        if (!target) return reply("❌ ᴩʟᴇᴀꜱᴇ ᴩʀᴏᴠɪᴅᴇ ᴀ ɴᴜᴍʙᴇʀ ᴏʀ ᴛᴀɢ/ʀᴇᴩʟy ᴀ ᴜꜱᴇʀ
.");
...
cmd({
    ...
    react: "📋",
    filename: __filename
}, async (conn, mek, m, { from, isCreator, isOwner, reply }) => {
    try {
        if (!isCreator) return reply("_❗This Command Can Only Be Used By My Owner!_");
